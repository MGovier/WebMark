...
Meteor.methods({
  ...
  addScheme: function (schemeObject) {
    ...
    if (!unitContainer) {
      Units.insert({units: [schemeObject.unitCode]});
    } else {
      Units.update(unitContainer._id, {$addToSet: {units: schemeObject.unitCode}});
    } 
  },
  deleteScheme: function (schemeId) {
    ...
    let schemeToDelete = MarkingSchemes.findOne({_id: schemeId});
    if (!schemeToDelete) {
    ...
    if (Meteor.userId() === schemeToDelete.creator) {
      ...
      Marks.remove({schemeId: schemeId});
    } else {
      throwError('not-authorized', 'The current user does not match the creator of this scheme.');
    }
  },
